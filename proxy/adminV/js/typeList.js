...
layui.config({
	...
	var app = new Vue({
		...
		methods: { //定义VUE中函数
			deletFiletype: function(tname, tid) {
				deletFiletype(tname, tid);
			}
	...
	//删除
	function deletFiletype(tname, tid) {
		layer.confirm('确定删除[' + tname + ']课程？', {
			icon: 3,
			title: '提示信息'
		}, function(index) {
			sendRequest("POST", "TypeServlet", {
				"m": "delet",
				"tid": tid
			}, function(result) {
				if(result.code == 0) {
	...
	function loadList() {
		...
		//加载页面数据
		sendRequest("POST","upload/TypeServlet",{
			"m": "mlist",
			"page": curr,
